...
public final class Task8 extends Task {
    ...
    public static boolean knightBoardCapture(int[][] board) {
        ...
        /*
         * int[8] rowsInInt - все ряды доски, представленные в виде 12-битного числа.
         *
        ...
        for (int i = 1; i < BOARD_SIZE; i++) {
            /*
             * Проверяем, есть ли у рядов с конями пересечение с рядами с возможными захватами.
             *
             * Тут же заметим, что в rowsInInt все ряды содержат максимум 10-битные числа (11-й и 12-й равны нулю,
             * так как максимальный сдвиг одного бита был 9, т. е. до 10-го бита), первый и второй бит тоже равны нулю,
             * так как производился сдвиг на 2 минимум, то есть все дополнительные 4 бита равны нулю и при конъюнкции
             * будут обнулять 1-й, 2-й, 11-й, 12-й биты чисел из canBeCapturedRows. Следовательно, сравниваться будут
             * фактические только биты с 3-го по 9-й, то есть те, что соответствуют изначальному ряду.
             *
             * Таким образом, расширение поля не влияет на ответ.
             * */
            if ((rowsInInt[i] & canBeCapturedRows[i]) != 0) {
                return false;
            }
        }
        return true;
